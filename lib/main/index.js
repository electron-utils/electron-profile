...
Type.on('register', (type, list) => {
    IpcPlus.sendToWins('electron-profile:type-list-changed', list);
});

IpcPlus.on('electron-profile:query-type-list', (event) => {
...
Profile.on('profile-changed', (data) => {
    IpcPlus.sendToWins('electron-profile:changed', data);
});
