...
class ProfileManager extends Events.EventEmitter {
    ...
    load (url, callback) {
        ...
        IpcPlus.sendToMain('electron-profile:load', url, (error, data) => {
            ...
            Type.each((type) => {
                if (!this.source_cache[data.id][type.name]) {
                    this.source_cache[data.id][type.name] = {};
                }
                Utils.copyObject(this.source_cache[data.id][type.name], data.source[type.name]);
            });
            ...
            profile.on('save', () => {
                ...
                IpcPlus.sendToMain('electron-profile:changed', {
                    id: data.id,
...
IpcPlus.on('electron-profile:changed', (event, data) => {
