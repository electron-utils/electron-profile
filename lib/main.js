 ...
 * Set schema
 */
profile.setSchema = function ( url, json ) {
...
class _Profile extends EventEmitter {
  ...
  set (key, value) {
    ...
    switch (error) {
      case schema.ERROR.NONE:
        this._data[key] = value;
        break;
      case schema.ERROR.TYPE:
        console.warn(`The profile type does not match - ${key} (${this._url})`);
        break;
      case schema.ERROR.NOT_DEFINE:
        console.warn(`The profile is not defined - ${key} (${this._url})`);
        break;
    }
